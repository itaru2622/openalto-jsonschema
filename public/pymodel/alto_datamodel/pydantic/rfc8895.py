...
class SubstreamID(BaseModel):
    __root__: ResourceID


class UpdateStreamControlEvent(BaseModel):
    control_uri: Optional[URI] = Field(None, alias='control-uri')
    started: Optional[List[SubstreamID]] = Field(None, min_items=1)
    stopped: Optional[List[SubstreamID]] = Field(None, min_items=1)
    description: Optional[str] = None


class IncrementalUpdateMediaTypes(BaseModel):
    class Config:
        extra = Extra.forbid

    __root__: Dict[constr(regex=r'^[0-9a-zA-Z-:@_.]{1,64}$'), str]


class AddUpdateReq(BaseModel):
    class Config:
        extra = Extra.allow

    resource_id: ResourceID = Field(..., alias='resource-id')
    tag: Optional[constr(regex=r'^[!-~]+$')] = None
    incremental_changes: Optional[bool] = Field(None, alias='incremental-changes')
...
class UpdateStreamCapabilities(BaseModel):
    class Config:
        extra = Extra.allow

    incremental_change_media_types: IncrementalUpdateMediaTypes = Field(
        ..., alias='incremental-change-media-types'
    )
    supported_stream_control: bool = Field(..., alias='supported-stream-control')


class AddUpdatesReq(BaseModel):
    class Config:
        extra = Extra.forbid

    __root__: Dict[constr(regex=r'^[0-9a-zA-Z-:@_.]{1,64}$'), AddUpdateReq]
